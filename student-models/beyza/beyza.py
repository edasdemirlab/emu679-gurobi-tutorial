...
inputs_file = 'orman_veri.csv'

# dosyalari okuyalim
orman_df = pd.read_csv(inputs_file)

# Sabit verilerin tanımlanması
analiz_alani = list(set(orman_df["analiz_alani"].tolist())) # ['Pencils', 'Pens'] # ürün seti listesi
receteler =list(set(orman_df["recete"].tolist())) # arz eden şehirler
...
for i in range(len(orman_df)):
    ...
    s[int(row["analiz_alani"])] = row["donum_alan"]
    
ij, p, t, g, w  = gp.multidict(orman_multidict)
